                            ...
                            courses.map(course => (
                                <div key={course.id} className="flex flex-col sm:flex-row sm:items-center py-5 gap-4">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="font-semibold text-base break-words truncate max-w-full" title={course.topic}>{course.topic}</div>
                                        <div className={`mt-1 text-[#7d7865] course-description truncate max-w-full`} title={course.details}>
                                            {course.details || 'No description'}
                                        </div>
                                    </div>
                                    <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto sm:min-w-[160px]">
                                        <button className="py-2 px-4 border border-[#bdb9a7] rounded text-[#bdb9a7] font-semibold bg-transparent hover:bg-[#f3f1e3] transition">Chat</button>
                                        <button className="py-2 px-4 rounded font-semibold bg-[#bdb9a7] text-white hover:bg-[#a7a083] transition">Start Session</button>
                                    </div>
                                </div>
                            ))
                        ) : (
        ...
        <div className="flex flex-col flex-1 min-h-screen bg-[#f8f7f4] font-[Montserrat]">
            <main className="w-full px-6 md:px-12 pt-23 pb-6">
                {notifications.map(notification => (
                    <NotificationBar key={`${notification.id}-${notification.reminderType}`} course={notification} reminderType={notification.reminderType} />
                ))}
